...
\title{Disentangled Models Achieve Better Retain--Forget Tradeoffs during Unlearning}
...
\begin{figure}[t]
  ...
  \caption{Retain--forget Pareto frontiers for each unlearning method across six models with varying SGTM exposure ($p$). Each frontier is constructed over all hyperparameter configurations; shaded regions show $\pm 1$ standard error across 5 seeds. The $x$-axis shows the increase in biology (forget) loss and the $y$-axis shows the increase in non-biology (retain) loss, both relative to each model's pre-unlearning baseline. Frontiers closer to the $x$-axis are better (more forgetting per unit of retain degradation). Models are colored from more entangled (purple, low $p$) to more disentangled (orange, high $p$). In all three methods, more disentangled models achieve better trade-offs, with the clearest separation in WGA. Note the different $y$-axis scales: RMU incurs minimal retain cost regardless of entanglement. The $x$-axis is truncated at $0.4$, corresponding to the gap between a no-biology baseline and the most disentangled model's pre-unlearning forget loss; beyond this point, further forgetting is not practically meaningful.}
