...
\title{Disentangled Models Achieve Better Retain--Forget Tradeoffs}
...
When knowledge that should be forgotten shares processing pathways, representations, or parameters with knowledge that should be retained --- in short, when the domains are \emph{entangled} in the model --- it is more difficult for unlearning methods to remove one without affecting the other.
At least, this is how the common wisdom goes.
And while this intuition is wide-spread in the unlearning literature \citep{barez2025openproblemsmachineunlearning}, experimental evidence supporting it has been lacking, especially in the context of large language models.

\citet{zhao2024what} show that forget sets more entangled with the retain set in representation space are harder to unlearn.
However, their methodology is limited to image models, and it holds the model and the unlearning methods fixed and swaps out different pairs of forget and retain sets to measure the difficulty of unlearning.
Other past work holds the model and the forget and retain sets fixed and swaps out different unlearning methods --- usually, the authors propose a new method that intuitively increases disentanglement and report that it performs better compared to existing methods \citep{tang2026logitslatentscontrastiverepresentation, chen2026clue}.
This experimental design conflates the effect of entanglement with the effect of the specific algorithmic intervention.
A more direct test would hold the unlearning method and the datasets fixed and instead vary the degree of entanglement in the model being unlearned.
To our knowledge, no prior work has conducted such a study, likely because it is difficult to control the degree of entanglement of two fixed datasets in a model.

In this paper, we do exactly this.
We use Selective Gradient Masking (SGTM)~\citep{shilov2025datafilteringknowledgelocalization}, an improved variant of gradient routing~\citep{cloud2024gradientroutingmaskinggradients}, to train a suite of models on English Wikipedia in which the knowledge of Biology is increasingly more and more localised to a specific set of parameters, disentangled from the others.
We then apply three common unlearning methods to all models in our suite: Weighted Gradient Ascent~\citep{wang2025rethinking}, Weight Divergence Regularization~\citep{siddiqui2025dormant}, and RMU~\citep{li2024wmdp}.
We find that more disentangled models consistently achieve better retain-forget trade-offs: at a fixed level of forgetting, the most disentangled model has $4\times$ lower retain cost than the most entangled model in WGA and RMU, and ${\sim}1.3\times$ lower in WDR.

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{assets/wiki_retain_forget_tradeoff.pdf}
  \caption{More disentangled models achieve better retain-forget trade-offs. The plot shows retain--forget Pareto frontiers for three unlearning methods across six models with varying SGTM exposure ($p$). Models are colored according to their Variance Entanglement Score (\cref{sec:entanglement}): the purple are less disentangled, the yellow are more disentangled.Each frontier is constructed over 4--6 hyperparameter configurations, shaded regions show $\pm 1$ standard error across 5 seeds. The axes are measured relative to each model's losses before unlearning.}
  \label{figure:unlearning}
\end{figure}

Our results confirm the widespread intuition that knowledge entanglement is a meaningful factor in unlearning difficulty, and that standard unlearning methods benefit from operating on models with more separated internal representations.
