...
For training disentangled models, we use SGTM~\cite{shilov2025datafilteringknowledgelocalization}, a recent method for training models so that pre-selected parameters are specialized to a target data domain. We closely follow the original SGTM training setup, including dataset, model architecture, the implementation of SGTM itself, and hyperparameters. Our sole modification is to vary the fraction of training during which SGTM is active, producing six models that span a range of entanglement levels.

\paragraph{Data.}
Following \citet{shilov2025datafilteringknowledgelocalization}, we train on English Wikipedia (approximately 3.7B tokens), using the same article-level topic labels derived from Wikipedia's \texttt{articletopic} classifier. Same as \citet{shilov2025datafilteringknowledgelocalization}, we assign each training example to one of three categories that govern how it is treated during SGTM training:
%
...
\end{itemize}
%
For evaluation, we hold out a biology test set (used to measure forget loss) and a non-biology test set spanning all other topic categories (used to measure retain loss), following the data splits of \citet{shilov2025datafilteringknowledgelocalization}. To reduce evaluation cost during our hyperparameter sweeps, we subsample $20\%$ of each test set and report all metrics on this fixed subsample.

% --- Model ---

\paragraph{Model.}
Following \citet{shilov2025datafilteringknowledgelocalization}, we train a 254M-parameter GPT-Neo-style transformer with 16 layers, hidden dimension 1024, 32 attention heads, and MLP dimension 4096. We adopt all training hyperparameters from \citet{shilov2025datafilteringknowledgelocalization}; see Table~\ref{tab:training-hyperparams} in Appendix \ref{app:hyperparams} for the full configuration. In each transformer block, we designate a small subset of parameters as the \emph{forget subnetwork}:
%
\begin{itemize}[nosep,leftmargin=1.5em]
...
SGTM~\cite{shilov2025datafilteringknowledgelocalization} is a modification to the training procedure that applies different interventions to the forward and backward passes depending on which of the three data categories ($\mathcal{D}_\text{forget}$, $\mathcal{D}_\text{retain}$, $\mathcal{D}_\text{unlabeled}$) an example belongs to. In the standard SGTM pipeline, these interventions are applied throughout training; our adaptation (described in the next paragraph) instead activates them for only a fraction of the training steps. The interventions are summarized in Table~\ref{tab:interventions}.
