...
\begin{itemize}
  ...
  \item \emph{route-ignore}: the remaining $90\%$ of retain and adjacent examples.
\end{itemize}
...
Route-ignore examples update all parameters via standard training.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    route-ignore & --- & --- & $\theta_\text{bio}$ and $\theta_\text{other}$ \\
    route-other & Set $\theta_\text{bio} = 0$ & --- & $\theta_\text{other}$ only$^{\dagger}$ \\
    \bottomrule
  \end{tabular}
  \caption{Training interventions in SGTM, applied to each example based on its routing category.
    ``---'' indicates standard~(unmodified) behavior.
