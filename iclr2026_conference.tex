...
\subsection{Training a Suite of Models with Varying Disentanglement}
\label{sec:training}
...
\paragraph{Data.}
Following the original paper, we train on English Wikipedia~\citep{TODO} (approximately 3.7B~tokens), using article-level topic labels derived from Wikipedia's \texttt{articletopic} classifier~\citep{TODO}, and splitting the data into three domains:
\begin{itemize}
...
In the original paper, SGTM was used for the whole training; we instead activate SGTM for only a fraction of the training steps, denoting this fraction $p\%$.
